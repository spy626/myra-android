...
new = '''                        // Whitespace is meaningful inside JSON strings and often arrives as
                        ...
                        val chunk = if (piece is String) piece else visibleText(piece)
                            ?: throw IllegalArgumentException("Cloudflare website stream has unsupported text; no files changed")'''
assert text.count(old) == 1, 'Cloudflare SSE whitespace anchor changed'
source.write_text(text.replace(old, new, 1), encoding='utf-8')

test = Path('app/src/test/java/com/myra/assistant/ui/workspace/WorkspaceCloudflareModelsWebsiteTest.kt')
assert not test.exists(), 'Regression file already exists'
...
class WorkspaceCloudflareModelsWebsiteTest {
    ...
    @Test fun streamingWhitespaceAndSplitJsonMakeOneCompleteThreeFileResult() {
        ...
        val streamed = chunk(raw.substring(0, middle)) + chunk(" ") + chunk(raw.substring(middle)) + end
...
''', encoding='utf-8')
print('STREAM_SAFETY_AND_REGRESSION_TESTS_STAGED')
