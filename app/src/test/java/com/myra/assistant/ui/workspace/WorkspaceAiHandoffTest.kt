...
class WorkspaceAiHandoffTest {
    ...
    @Test fun preparedHandoffQuotesBoundedSourceAndNeverWrites() {
        ...
        assertTrue(draft.prompt.contains("COPY oldText literally from that source"))
        assertTrue(draft.prompt.contains("Never take oldText from the goal, acceptance criteria, schema example"))
        assertTrue(draft.prompt.contains("Never assume any unseen text"))
