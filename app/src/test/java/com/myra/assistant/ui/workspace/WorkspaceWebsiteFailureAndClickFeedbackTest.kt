...
class WorkspaceWebsiteFailureAndClickFeedbackTest {
    ...
    @Test fun freshSiteTextFirstAndCompatibleJsonModeKeepSameFreeModelAndApprovedSource() {
        ...
        assertEquals(first.getJSONArray("messages").getJSONObject(1).toString(),
            compatible.getJSONArray("messages").getJSONObject(1).toString())
        val firstSystem = first.getJSONArray("messages").getJSONObject(0).getString("content")
        val secondSystem = compatible.getJSONArray("messages").getJSONObject(0).getString("content")
        assertTrue(firstSystem.contains("Return exactly THREE complete files"))
        assertTrue(secondSystem.contains("Return exactly ONE JSON object"))
        assertTrue(firstSystem.contains("Do not attach Explore click listeners"))
        assertTrue(secondSystem.contains("Do not attach Explore click listeners"))
