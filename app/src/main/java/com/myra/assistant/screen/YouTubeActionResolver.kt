...
/**
 * YouTube-only target policy. It is deliberately pure: AccessibilityService still owns
 * the actual click and post-action verification.
 */
object YouTubeActionResolver {
    data class VideoCandidate(
        val element: ScreenTargetCandidate,
        val title: String = element.label
    )
    ...
    fun resolveVideoTarget(
        ...
        val visible = candidates.filter {
            it.element.clickable && it.element.right > it.element.left && it.element.bottom > it.element.top
        }
        if (visible.isEmpty()) return Result.NotFound

        val query = tokens(targetText).filterNot(GENERIC).toSet()
        if (query.isNotEmpty()) {
            ...
            if (second != null && best.second - second.second < AMBIGUITY_MARGIN) {
                return Result.Ambiguous(listOf(best.first, second.first))
            }
        ...
        if (ordinal != null && ordinal > 0) {
            return ordered.getOrNull(ordinal - 1)?.let { Result.Selected(it, 1.0) } ?: Result.NotFound
        }

        if (position.equals("center", true) || position.equals("middle", true)) {
            val ranked = ordered.sortedBy {
                hypot(
                    (it.element.centerX - screenWidth / 2).toDouble(),
                    (it.element.centerY - screenHeight / 2).toDouble()
                )
            }
            val best = ranked.first()
            val diagonal = hypot(screenWidth.toDouble(), screenHeight.toDouble()).coerceAtLeast(1.0)
            val distance = hypot(
                (best.element.centerX - screenWidth / 2).toDouble(),
                (best.element.centerY - screenHeight / 2).toDouble()
            )
            return Result.Selected(best, (1.0 - distance / diagonal).coerceIn(0.0, 1.0))
        }

        return if (ordered.size == 1) Result.Selected(ordered.first(), 1.0)
        else Result.Ambiguous(ordered.take(3))
    }

    fun verifyVideoOpened(
        beforePackage: String?,
        afterPackage: String?,
        beforeSignature: String,
        afterSignature: String,
        afterLooksLikeVideo: Boolean
    ): Boolean {
    ...
    private fun titleScore(query: Set<String>, title: String): Double {
        ...
        val phraseBonus = if (normalized.contains(phrase)) 0.35 else 0.0
    ...
    private fun tokens(value: String?): List<String> = normalize(value.orEmpty())
    ...
    private val GENERIC = setOf(
        "open", "play", "tap", "click", "karo", "kholo", "chalao", "dabao",
        "wala", "wali", "video", "the", "called", "about", "this", "that"
    )
