...
/** Workspace text only. The caller must show an explicit provider/data consent dialog per request. */
internal object WorkspaceChatGateway {
    ...
    /** Bounded history from exactly one project; no source file, attachment or other project is read. */
    fun request(provider: Provider, key: String, messages: List<WorkspaceConversationStore.Message>): Request {
        ...
        val body = when (provider) {
            Provider.OPENROUTER_FREE -> openRouterBody(recent)
            Provider.GEMINI_FREE_TIER -> geminiBody(recent)
    ...
    fun openRouterBody(messages: List<WorkspaceConversationStore.Message>): String {
        ...
        messages.forEach { message ->
            ...
            entries.put(JSONObject().put("role", message.role).put("content", message.text))
        }
    ...
    fun geminiBody(messages: List<WorkspaceConversationStore.Message>): String {
        val entries = JSONArray()
        messages.forEach { message ->
            require(message.role == "user" || message.role == "assistant") { "Invalid chat role" }
            require(message.text.length in 1..WorkspaceConversationStore.MAX_MESSAGE_LENGTH) { "Invalid message size" }
