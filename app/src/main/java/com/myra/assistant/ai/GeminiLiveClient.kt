    ...
    private fun memoryProposalDeclaration(): JSONObject {
        val operation = JSONObject()
            ...
            .put("properties", JSONObject()
                ...
                .put("semantic_relationship", JSONObject().put("type", "STRING").put("description", "Required for ADD_RELATIONSHIP and REPLACE_RELATIONSHIP. Independently classify the exact strength expressed by the authoritative current user source_span; do not merely copy relationship.").put("enum", JSONArray(listOf("FRIEND", "GOOD_FRIEND", "BEST_FRIEND"))))
        ...
        return JSONObject()
            ...
            .put("description", "Interpret the current completed user turn into bounded independent AIRI semantic actions. Required by intent: ADD_RELATIONSHIP needs person+relationship+semantic_relationship; REMOVE_RELATIONSHIP needs person; REPLACE_RELATIONSHIP needs person+replacement_relationship+semantic_relationship; RENAME_ENTITY needs person+replacement_person; DELETE_ENTITY needs person; ADD_LINKED_FACT needs person+fact; ADD_EPISODE needs event_type+fact and participants when stated; ADD/UPDATE_GOAL need goal_title. semantic_relationship independently represents the exact strength expressed in source_span and must not be copied merely to agree with the requested operation. Every operation needs source_span+confidence+assertion_mode. source_span copies the shortest near-verbatim current-turn words; normalized fact meaning may be translated. Use USER_ASSERTED only for the user's own asserted proposition; hypothetical and reported speech must be marked. Put every critical literal in critical_literals. Questions are RECALL. Android validates structure, literals, attribution, lifecycle and safety and alone owns persistence.")
            .put("parameters", JSONObject()
                .put("type", "OBJECT")
